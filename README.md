...
- **Gmail spraudnis** — Chrome unpacked `extensions/gmail`: bez URL/Client ID; popup kartīte ar kontu, **Iziet** tikai kā ikona, centrētu loading, komandām un Drive/Gmail statusu (`{SYSTEM_NAME}`); paša sesija `chrome.storage.local`; Gmailā pogas tikai ja komandai ir Google Drive; savienojums caur `/auth/google-oauth/callback`; origini `www.tasqin.com` (pirmais) / `tasqin.com` / `localhost:3120`; `/api/extension/*` CORS atspoguļo derīgu `chrome-extension://` Origin (Bearer paliek auth, `CHROME_EXTENSION_IDS` nav vajadzīgs); admin modulis **Gmail spraudnis**
