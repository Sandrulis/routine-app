...
- **Gmail spraudnis** — Chrome unpacked `extensions/gmail` (MV3 `0.4.7`): bez URL/Client ID; popup kartīte ar kontu, **Iziet** tikai kā ikona, centrētu loading, komandām un Drive/Gmail statusu (`{SYSTEM_NAME}`); paša sesija `chrome.storage.local` (~30 dienas), neatkarīga no vietnes ielogošanās; Gmailā pogas tikai ja komandai ir Google Drive; **Turpināt ar Google** atver `/auth/gmail-plugin/login` → Google OAuth → `/auth/gmail-plugin/done` un done lapa nodod sesiju spraudnim; savienojums caur `/auth/google-oauth/callback`; noklusējums `www.tasqin.com` (ne `localhost`, ja production atbild); pēc spraudņa Reload jāpārlādē Gmail cilne; `/api/extension/*` CORS atspoguļo derīgu `chrome-extension://` Origin; admin modulis **Gmail spraudnis**
