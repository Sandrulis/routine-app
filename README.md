...
- **Auth** — `/login`, `/signup`, `/forgot-password`, `/update-password`, `/auth/confirm`; e-pasts un parole tikai ja **Resend** ir konfigurēts un aktīvs (bez tā nav Reģistrēties joslā, signup saites loginā un Atcerēties mani; `/signup` uz `/login`); login/signup pogas nāk no publiska RPC (`public_sign_in_methods`), nevis service role; reģistrācijas apstiprinājums un paroles atjaunošana iet kā HTML e-pasts no admin šabloniem (ne Supabase noklusējuma); uzaicinājuma signup (`/signup?invite=…`) ar Vārds/Uzvārds un bloķētu e-pastu, ja konta vēl nav; paroles lauki ar rādīt/paslēpt; **Turpināt ar Google** / **Turpināt ar Microsoft** (ja admin ieslēdzis OAuth) ved uz `/auth/google-oauth/sign-in` / `/auth/microsoft-oauth/sign-in` (GET, ar `?next=` no login lapas); signupā bez noteikumu ķeksīša — OAuth **nesasaista** esošu kontu tikai pēc e-pasta (kļūda `errors.auth_account_exists`); Microsoft prasa verificētu e-pastu; paroles lockout pēc neveiksmēm; **Cloudflare Turnstile** (ja admin ieslēdzis) - botu pārbaude e-pasta reģistrācijā un Google ienākšanā, ja vēl nav komandas; **Atcerēties mani** tikai loginā, pēc noklusējuma izslēgts (30 dienas ar ķeksi); ielogotam `/` ved uz dashboard
- **Gmail spraudnis** — Chrome unpacked `extensions/gmail` (MV3 `0.4.17`): bez URL/Client ID; popup kartīte ar kontu, **Iziet** tikai kā ikona, centrētu loading, komandām un Drive/Gmail statusu (`{SYSTEM_NAME}`); paša sesija `chrome.storage.local` (~30 dienas), neatkarīga no vietnes ielogošanās; Gmailā pogas tikai ja komandai ir Google Drive; **Turpināt ar Google** atver `/auth/gmail-plugin/login` → Google OAuth → `/auth/gmail-plugin/done` (bootstrap ticket → spraudņa sesija); **Atjaunot Gmail** caur ticket bridge → `/auth/gmail-plugin/start`; login ar `?next=` atgriež pie Gmail OAuth; OAuth login/signup caur `/auth/google-oauth/sign-in` un `/auth/microsoft-oauth/sign-in`; savienojums caur `/auth/google-oauth/callback`; noklusējums `www.tasqin.com`; pēc spraudņa Reload jāpārlādē Gmail cilne; `/api/extension/*` CORS; admin modulis **Gmail spraudnis**
