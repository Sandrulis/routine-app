...
- **Gmail spraudnis** — Chrome unpacked `extensions/gmail` (MV3 `0.4.3`): bez URL/Client ID; popup kartīte ar kontu, **Iziet** tikai kā ikona, centrētu loading, komandām un Drive/Gmail statusu (`{SYSTEM_NAME}`); paša sesija `chrome.storage.local` ar refresh ~30 dienām; Gmailā pogas tikai ja komandai ir Google Drive; savienojums caur `/auth/google-oauth/callback`; noklusējums un Login → `www.tasqin.com` (ne `localhost`, ja production atbild); pēc spraudņa Reload jāpārlādē Gmail cilne; `/api/extension/*` CORS atspoguļo derīgu `chrome-extension://` Origin; admin modulis **Gmail spraudnis**
