...
- **Gmail spraudnis** — Chrome unpacked `extensions/gmail`: bez URL/Client ID; popup kartīte ar kontu, **Iziet** tikai kā ikona, centrētu loading, komandām un Drive/Gmail statusu (`{SYSTEM_NAME}`); paša sesija `chrome.storage.local`; Gmailā pogas tikai ja komandai ir Google Drive; savienojums caur `/auth/google-oauth/callback`; origini `tasqin.com` / `www.tasqin.com` / `localhost:3120`; admin modulis **Gmail spraudnis**
