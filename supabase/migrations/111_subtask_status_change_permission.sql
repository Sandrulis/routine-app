-- Dedicated permission to change subtask status.
...
-- Keep prior access: default enabled for all existing roles.
update public.team_roles
set permissions = jsonb_set(
  coalesce(permissions, '{}'::jsonb),
  '{actions,subtasks.status.change}',
  to_jsonb(
    slug = 'owner'
    or coalesce((permissions -> 'actions' ->> 'subtasks.status.change')::boolean, true)
  ),
  true
);
