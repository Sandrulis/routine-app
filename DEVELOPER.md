...
**Reload / content script.** Pēc `chrome://extensions` → **Reload** vecā Gmail cilne zaudē kontekstu (`Extension context invalidated` pie `content.js` `sendMessage`). `content.js` `send()` to ķer un rāda `errors.extension_context_invalidated` (pārlādē Gmail ar F5). Pēc spraudņa Reload vienmēr **F5** Gmailā. Service worker Gmail cilnes **nepārinjicē** — pārinjicēts `content.js` noņemtu `#routine-gmail-root` lietotājam atvērtā modāļa vidū.

**Izvēles saraksts.** Katrs solis (saraksti → mape/uzdevums → apakšuzdevums) saņem **jaunu** `<ul>` caur `freshResults()`, nevis `results.innerHTML = ""`. Chrome inerciālo scroll animāciju tur pie mezgla, tāpēc atkārtoti lietots saraksts atvērtos pusceļā arī pēc `scrollTop = 0`; `resetResultsScroll()` papildus notur augšu vēl divus kadrus, kamēr pielikumi un **Jauns apakšuzdevums** maina augstumu. Inline pogas burts un teksts nāk no `systemName` (`applyInlineName`), ne no cietā teksta; ja ir augšupielādēts logotips, `applyLogoTo` burtu nomaina ar attēlu.
