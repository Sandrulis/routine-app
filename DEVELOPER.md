...
- `@dnd-kit` drag-and-drop (`DndContext` ar stabilu `id={useId()}`, lai SSR un klients nesakrīt `aria-describedby`)
- Ports: **3120**
...
- Ja aktīvas valodas > 1, galvenē `LanguageSwitcher` `variant="menu"` rāda karogu (izvēlnē karogs + nosaukums, noklusējums pirmais, tad alfabēts). UI valoda: URL prefikss (`/fi`, `/en`, …), tad `users.language_code`, tad viesu cookie ar `routine-app-language-chosen`, tad pārlūka `Accept-Language` (`matchAcceptLanguage`; `fi-FI` → `fi`, `nb-NO` → `no`), tad sistēmas noklusējums. Publiskām lapām bez prefiksa viesim ar citu pārlūka valodu ir 307 uz `/{code}` (meklētājprogrammas netiek novirzītas).
- Datumi UI: `useDisplayPreferences().formatDate()` / `formatDateTime()`. Efektīvās preferences: lietotāja (`users.week_start_day`, `date_format`, `date_separator`, `time_format`, `timezone`) ja norādītas, citādi `site_settings`. Laika josla: lietotājs → serveris (`Europe/Riga`); datumi ar laiku caur `Intl` ar efektīvo joslu; tīri `YYYY-MM-DD` bez nobīdes. Noklusējums: pirmdiena, `d.m.Y`, `.`, 24 h. Kalendāra popover (`date-picker-popover.tsx`) nedēļas dienas (`dates.weekday.*`) un mēneša virsraksts seko UI valodai, ne pārlūka locale; `inline` variants masveida joslai.
- Favicon: `app/layout.tsx` `<head>` + `generateMetadata.icons` no `siteHeadIconUrl` (`favicon_url` → `logo_url` → iniciāļu SVG ar `logo_color`). Ja `notification_favicon_url` ir iestatīts un ir nelasīti paziņojumi, `NotificationFavicon` pārslēdz cilnes ikonu; ja nav augšupielādēts, ikona nemainās.
...
| `cloud-storage/sanitize-folder-path.ts` | `sanitizeCloudFolderPath`, `defaultCloudFolderFromTeamName`, `LEGACY_CLOUD_FOLDER` | Drive/OneDrive mapju ceļi |
| `cloud-storage/default-folder.ts` | `resolveDefaultCloudFolderPath` | Jaunai integrācijai mape = komandas nosaukums |
...
| `format/ascii-file-name.ts` | `asciiSafeFileName`, `decodePercentEncodedName`, `transliterateToAscii` | Faila nosaukumi: `%20` atkodēšana, diakritika → ASCII (`ā`→`a`); Gmail atstarpes → `_` |
...
`app/components/app-nav.tsx` — fiksēta kreisā josla; zem 1280px slēpta, `AppShell` burger overlay (`sidebar-layout.ts` `xl`); zem 600px overlay ir 100% plats (`--app-sidebar-width` / `--app-todo-rail-width`). Rindu `+` / `...` / izvēršana overlay (zem `xl`) ir vienmēr redzama, datorā tikai hover. Kad hover nomaina rindas ikonu uz bultiņu, ikonai ir noapaļots fons. Izvērstam kokam katram slānim ir vertikāla līnija (`ml-[15px] border-l`). Rūpnīcas lietotāju blokā virsraksts ir virs līnijas. Galvenē `TeamSwitcher`: kreisajā avatārs (iniciāļi / logotips), pa labi nosaukums un amats (`teamRankLabel`); izvēlne portālā (`fixed`, virs overlay). Apgrieztiem nosaukumiem `OverflowTooltip` (zem 1024px tooltipi nerādās). `Tooltip` `matchMedia` ir kešots un paziņo nākamajā kadrā, lai loga platuma maiņa neizraisītu React ciklu. Klikšķis atver komandu sarakstu un **Pievienot jaunu komandu**. Hover uz rindas rāda `...` (Labot / Dzēst) pirms ķeksīša; mobilajā `...` vienmēr redzams; pēdējo komandu dzēst nevar. Bez komandas klikšķis uz pārslēdzēja atver komandas izveides modāli (nav bloķējoša); dashboard rāda tukšo stāvokli ar pogu un paziņojumu hintu.
...
- Komandas koks paliek redzams; klikšķis uz **Komanda** un lietotāju rindām atver `/team` (`canOpenTeamPage` vienmēr atļauj). Hover: `...` pirms izvēršanas chevron, tad `+`. `+` rādās tikai ar `team.invite` (`useStartTeamInvite`); `...` tikai ar `team.options` un vismaz vienu iekšējo opciju (lomas, veidnes vai Drive).
- Komandas `...`: **Komandas lomas**, **Šabloni** (`/templates`) — tikai ar `templates.manage` un `module_templates`; **Google Drive Integrācija** / **OneDrive Integrācija** — tikai ar `team.integrations.google_drive` / `team.integrations.onedrive` un attiecīgajiem moduļiem. Mapes/saraksta `+` → **Mape / Apakšmape** ar `folders.create`; **Pievienot šablonu** ar `templates.manage` (+ `module_templates`). **Abonementi** (`/team/billing`) ir sānjoslas rinda **aiz** Komanda (ne iekš koka), ja `payment_plans_enabled` un `team.settings.edit` (neatkarīgi no `nav.team`).
...
- Augšupielāde kokā: tikai ja `module_file_upload` + `files.upload`; apakšuzdevumos: `module_file_upload` + `files.upload.subtask` (skatīšanai `files.view`, pārsūtīšanai `files.forward` + `module_send_file`). Tikai katalogā esoši paplašinājumi (`isAllowedFileName`); `input accept` + toast `files.upload.rejected`; progressa overlay (`FileUploadOverlay`, XHR procenti uz Drive). Izslēgtam modulim faili kokā un apakšuzdevumā slēpti; mapes **Faili** logs un faila URL redirect.
...
| `/docs` | Publiskā dokumentācija (`DocsShell` layout + sānjosla; zem 1280px sānjosla slēpta, burger overlay); ieslēdz `/admin/docs`; kājene **Documentation** ja `docs_enabled` |

Auth: `/login` un `/signup` metodes (e-pasts, Google, Microsoft) nāk no RPC `public_sign_in_methods()` (`080`, `getPublicSignInMethods`) ar anon atslēgu — tikai `is_configured AND is_enabled` karogi, bez noslēpumiem. Service role login pogām nav vajadzīgs. E-pasta Ienākt iet caur Supabase `signInWithPassword` ar IP+e-pasta rate limit **un tikai ja Resend ir konfigurēts un aktīvs** (`isEmailPasswordAuthEnabled` vispirms RPC `email`, citādi From + API Key). Reģistrācija: `generateLink({ type: "signup" })` + HTML e-pasts no admin šablona `signup` (`sendSignupConfirmationEmail`); apstiprinājums `/auth/confirm?token_hash&type=signup` (`verifyOtp`). Uzaicinājuma signup: `/signup?invite={token}` ar Google/Microsoft (ja ieslēgts; `returnPath=/invite/{token}`, `login_hint`) un/vai Vārds/Uzvārds, bloķētu e-pastu un `account_exists` preview (`085`). Paroles lauki: `PasswordInput` (rādīt/paslēpt). E-pasta signup: `generateStrongPassword` aizpilda abus laukus (plain); kad lietotājs sāk rakstīt savu paroli, atkārtotais lauks tiek notīrīts; `PasswordStrengthMeter` + `isPasswordStrongEnough` (min. score 3 / Vidēja; arī `signUpWithPasswordAction` / `updatePasswordAction`). Google/Microsoft lietotājs bez e-pasta paroles lietotāja izvēlnē redz **Pievienot paroli** (bez pašreizējās paroles); pēc saglabāšanas var ienākt ar e-pastu un paroli tāpat kā e-pasta konts. Paroles atjaunošana: `generateLink({ type: "recovery" })` + šablons `password_reset`; saite ved uz `/auth/confirm` ar `next=/update-password`. Esošam apstiprinātam e-pastam signup un nezināmam e-pastam reset joprojām rāda vispārīgu veiksmi (nav enumerācijas). Bez Resend: e-pasta lauki nav, galvenē nav Reģistrēties, loginā nav signup saites un Atcerēties mani, `/signup` `redirect("/login")`; server actions atgriež `errors.auth_email_disabled`. **Cloudflare Turnstile** (`092`, `public_turnstile_config`, `requireTurnstileToken`): ja integrācija ir konfigurēta un **Aktīva**, e-pasta reģistrācija un Google ienākšana **bez komandas** prasa derīgu tokenu (jauns Google konts vai lietotājs bez `team_members`). Esošs Google lietotājs ar komandu ienāk bez Turnstile. Ja tokena nav, Google callback novirza uz `/login?pending=google` (vai signup) ar **Turnstile modāli** (`OAuthPendingTurnstileModal`); tokenu verificē vienreiz (`turnstileAlreadyVerified`). Google/Microsoft OAuth profilā ielasa `given_name`/`family_name` (vai sadala no `name`) un ieraksta auth metadata + `users.name`. Site Key publiski caur RPC; Secret tikai serverī. Login e-pasts, paroles atjaunošana un Microsoft OAuth Turnstile neprasa. Bez Turnstile auth strādā kā iepriekš. **Turpināt ar Google** / **Turpināt ar Microsoft** paliek neatkarīgi (admin **Integrācijas** OAuth): pogas ved uz GET `/auth/google-oauth/sign-in?next=…` / `/auth/microsoft-oauth/sign-in?next=…` (login forma nodod `?next=` no URL); callback `/auth/google-oauth/callback`, `/auth/microsoft-oauth/callback` — pēc profila no Google/Microsoft tiek izveidots vai atrasts Supabase Auth lietotājs un sesija. Verificēts OAuth e-pasts **ielaiž** esošu kontu ar to pašu adresi un `app_metadata.providers` papildina ar `google`/`microsoft` (`completeOAuthSignIn` / `findOrCreateOAuthUser`). Microsoft pieprasa verificētu e-pastu (OIDC / `id_token`, bez `userPrincipalName`). Pogas rādās tikai ja attiecīgā integrācija ir konfigurēta un **Aktīva**. Signupā OAuth **neprasa** noteikumu ķeksīti (tas paliek tikai e-pasta reģistrācijai). **Atcerēties mani** rādās tikai `/login` e-pasta formā; pēc noklusējuma izslēgts: bez ķeksīša sesija līdz pārlūka aizvēršanai; ar ķeksi 30 dienas (`httpOnly: false`, lai browser Supabase klients lasītu cookie; production HTTPS `Secure`). Ielogotam `proxy` `/`, `/login`, `/signup` un `/forgot-password` novirza uz `/dashboard`; neautentificēts lietotājs no aizsargātiem app ceļiem uz `/login`; landing `app/(marketing)/page.tsx` arī `redirect("/dashboard")`, ja ir sesija. Iziet ved uz `/`. Publiskajā galvenē ielogotam rādās **Atvērt lietotni**. MFA (TOTP) ir opcija `/settings/profile` visiem; ja faktoram ir `verified` un sesija nav `aal2`, lietotne rāda `MfaVerifyModal` pirms app čaulas. AAL `getMfaGate` ņem ar `getUser()` un JWT (`getAuthenticatorAssuranceLevel(jwt)`), ne `session.user`. `is_admin` pie `/admin` bez MFA tiek novirzīts enroll (`?mfa=required`); ar MFA, bet bez AAL2 - admin modālis. Paroles maiņa (`updatePasswordAction`) prasa `getCurrentUser` (CI `actions.ts` auth-guard).

Legal teksti: `app/lib/legal/documents.ts` (privātums, noteikumi, sīkdatnes). Juridiskā persona no `site_settings`: `legal_entity_name`, `legal_entity_reg_no`, `legal_entity_address`, `legal_email` (`115`); tukšs e-pasts izlaiž kontakta teikumu. Ja ir `legal_entity_name`, `/terms` ievads un 1.1–1.4 rāda uzņēmumu, reģ. nr., adresi un vietnes hostu (`getPublicSiteHost`). Politikas sedz apstrādātājus (Supabase, Vercel, Stripe, Google u.c.), integrācijas, konta dzēšanu (30 d.), sīkdatņu inventāru un EEZ pārnesumus. UI: `LegalDocumentView` ar **Saturs** sānjoslu.

## Google Search Console

Indeksējamās lapas: `/`, `/privacy`, `/terms`, `/cookies`, `/login`, `/signup`, un `/docs` plus katrs publiskais raksts ja `docs_enabled`. Pārējais (dashboard, saraksti, komanda, admin, iestatījumi, uzaicinājumi, paroles atjaunošana, `/api/`, `/auth/`, kalendārs) ir `noindex` + `robots.txt` `Disallow` + `X-Robots-Tag: noindex, nofollow`.
...
| `llms.txt` | `/llms.txt` (`public/llms.txt`) — statisks Markdown AI aģentiem (H1 + saites); `proxy.ts` to izlaiž, lai PageSpeed nenotimeoutē uz `/dashboard` |
...
| JSON-LD | landing `LandingJsonLd` (Organization, WebSite, SoftwareApplication + additionalType WebApplication, FAQPage; ja maksas plāni ieslēgti — Offer katram plānam uz `#pricing`; ja docs ieslēgti — CollectionPage ar TechArticle katram publiskajam rakstam) |
...
4. Pēc deploy atver `{NEXT_PUBLIC_SITE_URL}/robots.txt`, `{NEXT_PUBLIC_SITE_URL}/sitemap.xml` un `{NEXT_PUBLIC_SITE_URL}/llms.txt` - sitemap rādītajiem URL jāsākas ar to pašu origin.
...
Publiskās indeksējamās lapas: `/`, `/privacy`, `/terms`, `/cookies` un (ja `docs_enabled`) `/docs` + raksti — visās `LANGUAGE_CODES` valodās. `/login` un `/signup` ir `noindex`, bez hreflang un ārpus sitemap. Noklusējums ir `lv` bez prefiksa; pārējās ir `/{code}` (piem. `/en/privacy`, `/it`, `/sv`). `/lv` 308 uz versiju bez prefiksa. App maršruti paliek bez valodas prefiksa. `proxy.ts` iestata `x-ui-language`, lai title/description/canonical/H1 atbilstu URL, nevis tikai sīkdatnei. Norvēģu Bokmål: HTML `lang` un hreflang ir `nb-NO`, Open Graph `nb_NO`, publiskais ceļš `/no`. Sitemap iekļauj landing, legal un docs ar `lastModified` un hreflang; docs rakstu URL slug nāk no tulkojuma virsraksta attiecīgajā valodā (LV `/docs/sablonu-redaktors`, EN `/en/docs/template-editor`). OG/Twitter attēls 1200×630, lokalizēts tagline (`landing.seo.og_tagline`). Legal/docs lapām īsā `meta_description` + JSON-LD `WebPage`/`BreadcrumbList`. `meta keywords` netiek izvadīts.
...
- `routine-app-list-window-order`, `routine-app-history-pane` un `routine-app-status-group-collapsed` raksta tikai ar `preferences` piekrišanu
- Tabulas kolonnu redzamība un statusu grupu kārtošana ir `users.ui_preferences` (RPC `set_current_user_ui_preferences`, `153`); nav profila formas lauki; vecās sīkdatnes tiek vienreiz pārceltas
...
| Google Drive | `/team/google-drive` | Komandas OAuth, mapes ceļš; pieslēgts konts = faili uz Drive (`store_on_server = false`); `requireFrontendModules(module_google_drive, module_file_upload)` |
| Abonementi | `/team/billing` | Vadītāja vieta bez maksas; maksā tikai par vietām virs 1. Apmaksātās / aizņemtās / brīvās vietas, nākamais rēķins, checkout kopsavilkums (maksas lietotāji, summa, + PVN, Early bird sadalījums), mēneša/gada toggle, **Iegādāties 1 vietu** (Stripe prorata līdz cikla beigām) pirms uzaicinājuma, ja nav brīvas vietas; gaidošie **Samaksāt** vecām `pending_payment` rindām; neizmantotā vieta cikla beigās nokrīt no nākamā rēķina; Checkout success ar `session_id` apstiprinājumu (mount guards pret dubultu toast); **Atteikties no abonementa** perioda beigās (`cancel_at_period_end`) un atjaunošana (zaļa **Atjaunot abonementu** poga arī `TeamSubscriptionEndingBanner`); salokāma abonementa pārvaldība kopsavilkuma kartē (`fa-angle-down`); `TeamSubscriptionEndingBanner` + `useTeamBillingLiveSync` (Stripe webhook/polling bez refresh); vadītājs / `team.settings.edit` / `is_admin` |
| Personīgie uzstādījumi | `/settings/profile` | lasāms profila kopsavilkums + datumu/laika preferences (nedēļas sākums, formāts, atdalītājs, 12/24 h); vārdu/uzvārdu labo lietotāja izvēlnē; **Dzēst kontu** (`AccountDeletionSection`) — 30 dienu deaktivizācija, auto-atjaunošana pieslēdzoties (`reactivatePendingDeletion`), cron `purge_scheduled_account_deletions` |
| Administrācija | `/admin` | kategoriju izvēlne ar hover dropdown (Cilvēki, Katalogs, Sistēma); tikai `is_admin` |

Ceļa josla: `app/components/page-breadcrumb.tsx` (`fixed` pie ekrāna augšas). Katram posmam ikona pēc tipa (`workItemIcon` mapei/uzdevumam/apakšuzdevumam, `ListBadge` sarakstam, `FileIcon` failam) - arī vecākiem posmiem, ne tikai aktuālajam. Ja mapei vai uzdevumam ir vairāk nekā viens kaimiņš, klikšķis atver `PathItemMenu` ar tiem pašiem kaimiņiem; mapi var izvērst un atvērt citu mapi vai uzdevumu. Izvēlne ir `w-max` (līdz `max-w-64`); bultiņas vieta ir tikai rindām ar bērniem. Katram izvērstam slānim ir `border-l`. Labajā malā `AdminPanelButton` (`fas fa-users-cog`, tikai `is_admin`), `NotificationsMenu` (zvaniņš) un valodas karogs, ja aktīvas valodas > 1 (`LanguageSwitcher` portālā). Zem 1280px kreisajā burger; zem 768px PATH slēpts un `TeamSwitcher compact` (avatars + nosaukums). `Tooltip` / `OverflowTooltip` zem 1024px nerādās.
...
| Ceļš | Saturs |
|---|---|
| `/admin/users` | Visi `public.users`: pievienot, labot, dzēst; `is_admin` slēdzis (neapstiprinātam e-pastam izslēgts); pēdējā tiešsaiste; UI valodas kods; pēdējā IP un valsts (`142`, tikai admin); aktīvie/kopējie darāmie (`listAdminUsers`, `149` admin select); kārtošana pēc vārda; `AlphabetFilter` (Visi / A–Z / #) |
| `/admin/teams` | Visas `teams`: pievienot, labot, dzēst (kaskāde uz darba datiem); skaitļi sarakstiem, aktīvajiem/izpildītajiem uzdevumiem un apakšuzdevumiem (`listAdminTeams`, `148` admin select); VIP zvaigzne lietotāju modālī (`setAdminTeamVipAction`, `is_vip`); kārtošana pēc nosaukuma; `AlphabetFilter`; izveides datums un relatīvais laiks zem nosaukuma |
...
| `/admin/docs` | Publiskā dokumentācija (`site_docs_*`): kategorijas ar ikonu, apakškategorijas ar Markdown (kods, YouTube, attēli), slogans, redzamība, DnD `sort_order`; slēdzis `docs_enabled` rāda kājenes saiti Documentation. Teksts ir katrā sistēmas valodā (sākumā no noklusējuma valodas, pēc tam rediģējams); attēli paliek kopīgi apakškategorijai. Poga ievieto `{SYSTEM_NAME}`; publiski tas kļūst par `site_settings.system_name` treknrakstā un lielajiem burtiem (`app/lib/docs/placeholders.ts`) |
| `/admin/announcements` | Globāli paziņojumi (`site_announcements`): virsraksts/teksts visās valodās, termiņš, ieslēgums (`AdminAnnouncementsManager`). Lietotājiem aizverams bloks (`GlobalAnnouncementsBanner`); aizvēršana raksta `routine-app-announcement-seen_<id>` sīkdatni līdz termiņam |
...
| `/admin/settings` | `site_settings`: sistēmas nosaukums, juridiskais e-pasts un juridiskā persona (`legal_email`, `legal_entity_*`, privātums/noteikumi), slogans, logotips/favicon un paziņojumu favicon (data URL) vai iniciāļu avatārs ar `logo_color`, nedēļas sākums, datuma formāts/atdalītājs, 12/24 h, servera laika josla; ja Resend konfigurēts — From/Reply-To (`AdminSettingsForm` + `saveSiteSettingsAction`); hinti zem laukiem |
...
- Darbu atslēgas: `subtask_start_reminder` (sākums), `subtask_due_reminder` (termiņš), `purge_scheduled_account_deletions` (kontu dzēšana pēc 30 dienu termiņa)
...
- Dublēšanu bloķē `app_notifications` (`kind` `start` / `due`) tajā pašā vietējā dienā; e-pastu sūta `sendCronReminderEmails` → `flushPendingNotificationEmails`, kas pievieno arī citus nesūtītos paziņojumus tam pašam saņēmējam
...
| `module_gmail_plugin` | Gmail Chrome spraudnis (`extensions/gmail` `0.4.66`): sesija, komandu pārslēgšana, e-pasta pievienošana ar piezīmi katram failam, jauns apakšuzdevums no Gmail. Faili uz komandas Drive un/vai OneDrive (pēc ieslēgtajiem moduļiem). **Turpināt ar Google** → `/auth/gmail-plugin/login` ar identity + `gmail.addons.current.message.readonly` (offline) vienā piekrišanā caur **Google Plugin** integrāciju; callback `/auth/google-plugin/callback` saglabā arī `user_gmail_connections`. E-pastu un pielikumu baitus lasa no atvērtās Gmail cilnes; saraksta nosaukumus un izmērus papildina no Gmail API, ja savienojums ir. Ja kontam ir TOTP, done prasa 2FA (AAL2), tad bootstrap ticket → `POST /api/extension/bootstrap-from-ticket` (post-MFA refresh; AAL1 noraida `accessTokenNeedsTotpChallenge`). Done zaļais stāvoklis tikai pēc sesijas capture. **Atjaunot Gmail** (reconnect) → ticket bridge `/auth/gmail-plugin/bridge?t=…` → `/start`; production vajag `INTEGRATION_SECRETS_KEY`. Admin slēdzis tikai ja **Google Plugin** ir ieslēgts. | Spraudnis rāda, ka modulis izslēgts |
...
| `module_timetracking` | Katram Check List punktam Sākt / Pauzēt / Apturēt (`154`, ieslēgts). Stop uzliek `done` un rāda sākumu, beigas un kopējo aktīvo laiku (pauzes neskaita). Nosaukums `frontend_modules.label.module_timetracking` | Pogas nav; saglabātie `timeLogs` paliek JSON |
| `module_factory` | Maksas modulis (`155`, ieslēgts, tikai ne-bezmaksas plānos). Maksājošas vai VIP komandas team dropdown un **Komanda** `...` rāda **Rūpnīca** → `/team/factory`. Lietotāji glabājas `team_factory_users`, nevis `team_members`, tāpēc par viņiem nav jāmaksā vieta. Publiskā lapa `/factory`: ielogošanās un saraksts ar koplietotiem apakšuzdevumiem. Virsraksts ir mape (`hasFolder`); ja apakšuzdevums nav mapē, rāda uzdevuma nosaukumu. Apakšuzdevuma nosaukums paliek apakšā, ja tas atšķiras. Nosaukuma laukā, kad tas mainīts, labajā malā ir absolūta saglabāšanas ikona (`actions.save`), kas nepabīda rindu. Apakšuzdevuma nosaukumam ir uzgriežņu ikona (`157` `factory_shared`), pelēka izslēgta un zaļa ieslēgta, ja modulis ir aktīvs un komanda ir maksājoša vai VIP. Atvērts bloks paliek `/factory?job=`. Punktu statusu var mainīt tikai aktīvajā posmā, ja to ir uzsācis darbinieks un tas nav pabeigts. Slēgts apakšuzdevums sarakstā nav | Izvēlnes opcijas un ikonas nav; `/team/factory` redirect uz `/dashboard`; `/factory` rāda, ka modulis ir izslēgts |
| `module_todo` | Labajā malā personīga **Darāmo saraksts** sleja (`user_todos`); klikšķis uz teksta labo nosaukumu uz vietas (`renameTodo` / `updateUserTodoTitle`, Enter saglabā, Escape atceļ); atvērtos ierakstus var vilkt, lai mainītu prioritāti (`sort_order`); atķeksētais pazūd no saraksta (done + `completed_at`); arhīva poga rāda pabeigtos ar datumu; X paslēpj sleju, ikona ceļa joslā atver atpakaļ (badge ar atvērto skaitu, ja sleja nav redzama) | Sleja un landing kartīte nav |
...
- Uzdevumu paziņojumos ceļš (`task_path`, `139`) — saraksts / mape / vecāks; live no workspace, fallback uz saglabāto snapshot
...
- **`appendNotifications`** pirms insert filtrē pēc `user_notification_preferences` (trūkstoša rinda = ieslēgts); pēc insert `sendNotificationEmailsAction` ieplāno digestu (`scheduleNotificationEmailFlush`, 20 s pauze). `flushPendingNotificationEmails` apvieno visus nesūtītos (`email_sent_at` null, `140`) vienā HTML e-pastā uz saņēmēju (šablons `notification`), identiskus `kind`+`href`+nosaukumu saspiež vienā rindā; cron atgādinājumi iet caur to pašu flush (`quietMs: 0`). Izņēmums: `team_invite` (uzaicinājuma šablons). Resend izslēgts = tikai in-app
...
- **`seat_open`** — komandas vadītājam (`owner` / `team.settings.edit`) pēc lietotāja noņemšanas, ja paliek brīva apmaksāta vieta; `task_title` = cikla beigu datums; `href` `/team/billing`; globālais baneris `OpenPaidSeatBanner` (**Uzaicināt** atver `TeamInviteModal` caur `REQUEST_TEAM_INVITE_EVENT`)
...
- Stāvoklis: `app/lib/use-notifications.ts` lasa/raksta/dzēš `app_notifications` tabulu; `notification-unread-store.ts` dala nelasīto skaitu ar `NotificationFavicon`

## Apakšuzdevuma modālis

`app/components/subtask-detail-modal.tsx` + `AppModal` (`dirty` no nosaukuma, apraksta, datumiem — **statuss, atbildīgie un čeklisti dirty neskaita**). Statusa un atbildīgo (personas/grupas) maiņa esošam apakšuzdevumam `updateTask` uzreiz. Čeklisti esošam uzdevumam persistējas uzreiz (tekstam debounce). Saglabāt **neaizver** modāli un nepāriet uz citu lapu; aizver X / ESC / Atcelt. Pēc jauna apakšuzdevuma izveides paliek edit mode. Poga **Pievienot jaunu** (tikai plus + tooltip `actions.add_new`) rādās, kad ir nosaukums un Saglabāt nav aktīvs; klikšķis atver tukšu formu tajā pašā modālī. Uzdevuma galvenē (`list-summary` / `task-detail-page`) aiz nosaukuma un arhīva ikonas `+` (`subtasks.add.title`) atver jaunu create modāli (`createFor`). `headerMeta` rāda `izveidots {date}` no aktivitātes `kind === "created"` (`useDisplayPreferences().formatDate`); jaunam nesaglabātam apakšuzdevumam datums nav. `headerSubtitle` rāda `TaskLocationPath` (`getSubtaskLocationSegments` / `getParentTaskLocationSegments`) — saraksts un mapes līdz vecākam uzdevumam, katram posmam tipa ikona. Atverams arī no saraksta loga un `/lists` kopsavilkuma, bez navigācijas. Divkolonnu izkārtojums: kreisā kolonna nosaka augstumu; vēsture labajā ir `absolute` pret kreiso un ritinās iekšēji (`ScrollableHistoryList` — fade + chevron, kad ir overflow).
...
| Statuss | `StatusControl` — krāsaina poga; nākamais (`fa-angle-right`) un Check. Tabulā bez hover tikai nosaukums, hover rāda `>` pie pogas un aiz tās Check / pārvietot / dzēst (bez animācijas, nepārklāj nosaukumu). Klikšķis uz nosaukuma atver picker. `comment` līmenī statusu drīkst mainīt izpildītājs. Ja ir čeklista punkti, zem pogas zaļa progresa josla; slēgto grupu un Check tikai pie 100% |
| Projekts, atbildīgie | Saraksta badge, `AssigneeCell` (izvēlne `createPortal` uz `document.body`, lai netiktu nogriezta tabulā) |
| Check List | `TaskChecklists` — pirms pielikumiem; unikāls React `key` (`checklists-…`, atsevišķi no `attachments-…`); vairāki saraksti ar nosaukumu; nākamais tukšais punkts parādās pēc teksta; atzīmēšana ar ķeksīti. Vismaz divus sarakstus un vismaz divus punktus var vilkt (`@dnd-kit`, rokturis). Nosaukumu un punktu tekstu saglabā pēc `blur` vai 500 ms bez rakstīšanas (ne katrā burtā). Tukšs sākumā sakļauts (izvēršams); ar vismaz vienu sarakstu atvērts. `forceCollapsed` ja `module_checklist` izslēgts. Struktūru labo `canEditTasks`; punktus atzīmē arī `canChangeStatus`. Ja `module_timetracking`, katram punktam Sākt / Pauzēt / Apturēt (`activeTimer`, `timeLogs` uz punkta). Stop uzliek `done` un pogas vietā rāda trīs kolonnas: sākums, beigas, kopā |
| Pielikumi | `TaskAttachments` — tikai ja `module_file_upload`; drag-and-drop vai **pārlūko** prasa `files.upload.subtask`; bez pieejas rāda `errors.extension_subtask_upload_forbidden`; zem zonas `files.upload.allowed_types`; `accept` no kataloga; kartītes ar ikonu/krāsu; garš nosaukums paliek kartītes platumā (`truncate`); apgrieztam `OverflowTooltip` (pilns vārds + paplašinājums); piezīme (`task_files.note`) kā tooltip uz kartītes, `…` → **Piezīme**; X augšējā labajā stūrī dzēš caur `ConfirmModal`; `...` → **Pārsūtīt failu** ja `module_send_file` un Resend; `disabled` bez `edit` / `full_edit`. Tukšs sākumā sakļauts (izvēršams); ar failiem atvērts |
| Faila `...` | `CreateItemMenu`: Apskatīt, Lejupielādēt, Pārsaukt, Piezīme, Dzēst. Klikšķis uz kartītes arī atver apskati. Izvēlne ar `data-app-modal-ignore-backdrop`, lai neaizvērtu apakšuzdevuma modāli |
| Dzēst failu | Tikai `ConfirmModal` (`files.delete.*`) |
| Pārsaukt failu | `NameFormModal` (`files.edit.*`) |
| Faila piezīme | `FileNoteModal` (`files.note*`); rādās kā tooltip uz pielikuma kartītes un mapes **Faili** logā hover |
...
Failu metadati: `TaskFile` / `ListFile` (`size`, `hasContent`, `googleDriveFileId`, `oneDriveFileId`, …). Binārie faili **bez** Postgres `content` (`123`); baiti paliek Google Drive un/vai OneDrive. Maziem teksta/e-pasta failiem (`txt`/`html` u.c.) saturs atļauts DB (`137`) preview vajadzībām. Atļautie paplašinājumi: `file_type_extensions` (sākumā pdf, dwg, Office + attēli + txt/html + zip/rar; `071`/`072`/`075`); `app/lib/file-types.ts` + `FileTypesProvider`. Ikona/krāsa: `FileIcon`. Progress: `FileUploadOverlay`. Klikšķis: `FileViewerProvider` — bildes/PDF/txt modālī (`isBrowserPreviewableFile`), pārējie lejupielādējas; saturs caur `/api/work-files/content` (DB → Drive → OneDrive); kamēr notiek ielāde/lejupielāde, rādās overlay un toast `files.download.started`. PDF: `blob:` iframe bez `sandbox` un `#navpanes=0`. E-pasta `.txt`/`.html`: `buildEmailPreviewDocument`. Apakšuzdevuma kartītēs `formatFileSize`.
...
- Tabula: `TableColumnsBadge` atver labo sleju ar slēdžiem (nosaukums paliek); `table-fixed` bez `minWidth`, nosaukums sašaurinās. Statusa kolonna ir `garākais nosaukums ch + 9rem` (`statusColumnWidth`), lai hover pogas (`>` / Check / snooze) paliek šūnā. `+` galvenē pievieno saraksta kolonnu (`work_lists.custom_columns` / `column_order`, vērtības `work_tasks.custom_fields`). Pielāgotās var pārsaukt ar dubultklikšķi un dzēst. Redzamība un statusu kārtošana: `users.ui_preferences` (sinhronizējas starp ierīcēm, nav profila UI). Statusu grupu sakļaušana: cookie `routine-app-status-group-collapsed`. Statusu grupu kārtošanas badge: `StatusGroupSortBadge`
...
`app/lib/list-access.ts` — līmeņi `full_edit` | `edit` | `comment` | `view`. Noklusējums jaunam un esošam sarakstam: **`full_edit`**. Vadītājs, `is_admin` un izveidotājs vienmēr `full_edit`.
...
| Nav konta | Preview `account_exists = false` (`085`) → redirect `/signup?invite={token}`: Google/Microsoft (augšā) un/vai Vārds/Uzvārds/Parole, e-pasts disabled+prefill; OAuth `next=/invite/{token}`; metadata `given_name` / `family_name` / `name` / `full_name` |
...
| Noņemt lietotāju | Pieeja `team.members.remove` (`canRemoveTeamMember` / `assertCanRemoveMember`). `/team` sarakstā poga arī apstiprinātiem, pa kreisi no pēdējās tiešsaistes zīmes; `/team/[id]` tāpat. Ne vadītāju un ne sevi. Noklusējuma `member` lomai izslēgts; vadītājam un `is_admin` ir |
| Iecelt vadītāju | Tikai pašreizējais `owner` (`canAppointTeamLeader`). `/team` un `/team/[id]` apstiprinātam lietotājam (ne pending). RPC `transfer_team_leadership` apmaina lomas un `teams.created_by` |
...
**Pamest komandu:** apstiprināts lietotājs (ne vadītājs) var pats aiziet — `removeTeamMemberAction` self-leave, UI `TeamLeaveSection` (`/team`, `/settings/profile`). Vadītājs vispirms ieceļ citu vadītāju.
...
Aktīvais lietotājs raksta `last_online_at` visās savās `team_members` rindās ik pēc 90 s (`touchMemberOnline` → RPC `touch_current_member_online`, `063`/`143`). Komandas sānjosla ik 30 s pārlasa citu lietotāju `last_online_at`, lai zaļais punkts nebūtu tikai tekošajam kontam. Relatīvais laiks UI: `NowProvider` / `useNow()` (60 s). Sānjoslā aktuālais lietotājs rādās kā tiešsaistē. Admin lietotāju tabulā avots paliek `team_members.last_online_at` (ne `last_sign_in_at`); zem e-pasta arī UI valodas kods un pēdējā IP ar valsti (`user_last_ips`, `142`).

## Project structure

```
proxy.ts                          # Sesijas refresh, ielogota novirzīšana, publisko lapu valodas prefiksi (`/en`, `/de`, …); `/lv` → `/`; kanoniskā hosta 301 ar CORS, ja Origin ir `chrome-extension://`; CSP nonce uz request un response (`buildContentSecurityPolicy`); izlaiž `robots.txt`, `sitemap.xml`, `llms.txt`
next.config.ts                    # HSTS, X-Frame-Options, …; `canonicalHostRedirectRules()` izlaiž `/api/extension/*` (CSP ir `proxy.ts`, ne te)
public/
  llms.txt                        # /llms.txt — Markdown kopsavilkums AI aģentiem (neiet caur proxy)
app/
  ...
  robots.ts                       # /robots.txt — publiskās lapas, bloķē app/API
    sitemap.ts                      # /sitemap.xml — landing + legal × visām LANGUAGE_CODES; `/docs` + raksti ja docs_enabled
  ...
  (docs)/
    docs/                         # Publiskā dokumentācija (layout DocsShell + sidebar); ieslēdz /admin/docs
    docs/[category]/[article]/    # raksts + loading.tsx; sānjosla paliek layoutā
  (app)/
    ...
    admin/                        # users, teams, roles, statuses, file-types, languages, translations, docs, announcements, modules, payment-plans, integrations, email-templates, settings
  ...
  components/
    ...
    site-footer.tsx               # Publiskā kājene; Documentation saite, ja docs_enabled
    docs-shell.tsx                # Publiskais docs layout: sānjosla, burger overlay zem 1280px, raksta loading
    docs-sidebar.tsx              # Docs kategorijas/apakškategorijas; overlay (`hidden xl:flex`)
    docs-article-content.tsx      # Publiskā raksta saturs (virsraksts, Markdown, pager)
    docs-markdown.tsx             # Markdown: teksts, koda bloki, YouTube, attēli (mobilē 100%, datorā 50%; vispirms `?preview=1`, tad pilnais; klikšķis pietuvina)
    docs-article-pager.tsx        # Iepriekšējais / nākamais raksts pēc sort_order
    docs-article-images.tsx       # Admin raksta attēlu galerija, ievietošana un upload progress uz sīktēla
    admin-docs-categories.tsx     # /admin/docs: ieslēgums, kategorijas CRUD + DnD + valodu cilnes; loading, kad atver kategoriju
    admin-docs-articles.tsx       # /admin/docs/[id]: apakškategorijas, saturs, attēli (kopīgi visām valodām), DnD, `{SYSTEM_NAME}` ievietošana, valodu cilnes tulkojumiem; loading, kad atver rakstu; FileUploadOverlay augšupielādei
    docs-source-language-notice.tsx # Docs valodu cilnes admin formās
    admin-announcements-manager.tsx # /admin/announcements: globāli paziņojumi, valodu tabi, termiņš, ieslēgums
    global-announcements-banner.tsx # Aizverams paziņojuma bloks lietotnē (sīkdatne līdz termiņam)
    ...
    change-password-modal.tsx     # Paroles maiņa (email login); PasswordInput + stipruma josla
    ...
    team-switcher.tsx             # Komandas pārslēdzējs, CRUD; izvēlne portālā; compact galvenē mobilajā
    ...
    team-plan-inactive-banner.tsx  # sarkans baneris vadītājam, ja abonements neapmaksāts
    team-billing-member-paywall.tsx # blur + modālis parastiem lietotājiem (komandas pārslēgšana)
    ...
    date-picker-popover.tsx       # Termiņa kalendārs; `inline` masveida joslā; dienu saīsinājumi pēc UI valodas
    subtask-bulk-bar.tsx          # Masveida josla: saraksta/uzdevuma statusi, pienākumi, inline kalendārs
    ...
    task-checklists.tsx           # Check List; DnD secība; punkta time tracking, ja modulis ieslēgts
    ...
    page-breadcrumb.tsx           # Fiksēta ceļa josla + admin ikona + zvaniņš + valoda; mobilajā burger + TeamSwitcher compact
    ...
    admin-settings-form.tsx       # Sistēmas uzstādījumi + logo/favicon/paziņojumu favicon
    ...
    admin-team-members-modal.tsx  # Komandas lietotāji + VIP zvaigzne
    admin-teams-manager.tsx       # Komandu saraksts + plāna kolonna + VIP
    branding-image-field.tsx      # Logo/favicon drop zona
    notification-favicon.tsx      # Cilnes ikona, kad ir nelasīti paziņojumi
    language-switcher.tsx         # Valodas pārslēdzējs; `menu` variants portālā (`fixed`)
    notifications-menu.tsx        # Paziņojumu panelis; uzdevuma ceļš zem teksta
  ...
  lib/
    ...
    cloud-storage/                # sanitizeCloudFolderPath, parsePathParts, queue-upload, message-key (Drive/OneDrive)
    ...
    landing/features.ts           # Landing fīčas un hero teksts pēc frontend moduļiem; `planIncludesLandingFeature` salīdzinājumam
    landing/faq.ts                # Landing FAQ pēc frontend moduļiem (`module_send_file` u.c.) un cenu sadaļas
    landing/pricing.ts            # Kad rādīt landing cenas (`payment_plans_enabled` + ir plāni)
    ...
    task-checklists.ts            # Čeklistu tipi, progress, punkta taimeris (`timeLogs` / `activeTimer`)
    ...
    sync-folder-template-statuses.ts # Šablona statusi jau izveidotās mapēs: trūkstošie, secība, paslēptie
    ...
    google-drive/                 # OAuth, status, Drive upload, content-url, context, team settings
    onedrive/                     # OAuth, status, upload, content-url, context, rename
    payment-plans/                # helpers + repository + team-plan (katalogs, cenas, trial, Early bird, moduļu filtrs)
    billing/                      # vietu skaits, invite gate (pay-first), Checkout/Invoice, webhook sync, checkout-estimate; team-access-state, use-team-billing-access; reset-all-team-billing.mjs (`097`/`098`/`099`); `use-start-team-invite.ts` (auto extra seat)
    ...
    site-admin/branding.ts        # logo/favicon data URL, iniciāļu favicon SVG, paziņojumu ikonas maiņa
    notification-unread-store.ts  # Kopīgs nelasīto paziņojumu skaits (cilnes ikona)
    ...
    auth/                         # actions (generateLink + Resend HTML), email-password, generate-strong-password, password-strength, auth-confirm-link, MFA (`getMfaGate`: AAL + admin TOTP lookup), OAuth, remember-session
...
supabase/migrations/              # 001–150: shēma, admin, work data, Drive, drošība, ielādes ātrums, e-pasta šabloni, Gmail spraudnis, publiskie login karogi, extra valodas, legal_email, invite preview `account_exists`, `file_forwarded` vēsture, Resend email id indekss, user feedback + feature votes, cron jobs, hashed tokeni, timezone batchi, Turnstile, payment plan max_members, free/paid plānu seed, Stripe vietu norēķini, brīvas vietas paziņojums, Early bird vietu pool, `billing_due`, noklusējuma lomu labels visās valodās (`101`), publiskā dokumentācija (`118`–`121`, `150`), VIP (`122`), failu `content` strip (`123`), `onedrive_file_id` (`124`), globāli paziņojumi (`125`), personīgs uzdevumu Atlikt (`134`), paziņojumu e-pasta digest (`140`)
...
Lietotne lasa sarakstus, uzdevumus, komandas, todo, paziņojumus un failus no Postgres (migrācija `005_work_data.sql` un tālākās). RLS: `is_team_member(team_id)` / `is_team_owner(team_id)`; saraksta darbībām papildus `work_list_has_access(list_id, min_level)` (`022`). Anon piekļuve liegta. Komandas biedri ar `team_members.user_id = auth.uid()` redz tos pašus datus, ja saraksts nav privāts vai viņiem ir viewer/lomas rinda. `public.users` paliek konta profils + `is_admin`; komandas loma ir `team_members.role` / `role_id` → `team_roles`. Migrācijas `001`–`004`: `is_admin`, pirmais **apstiprinātais** reģistrētais ir admin (`145`/`146` — neapstiprināts e-pasts paliek lietotāju sarakstā bez `is_admin`), `current_user_is_admin()`, noņemts liekais `users.role` / `manager_id` un vecās neizmantotās project/task tabulas. `db:test` apstiprina API projektu, bet Postgres pieprasa pareizu datubāzes paroli (ne `anon` atslēgu).
...
Atsevišķs Google Cloud projekts Gmail Chrome spraudnim (`site_integrations.google_plugin`, migrācija `136`). Scopes: `openid email profile` + `gmail.addons.current.message.readonly` (nav `gmail.readonly`). Redirect: `/auth/google-plugin/callback` visiem `KNOWN_SITE_ORIGINS`. Admin **Integrācijas** → **Google Plugin**: Client ID/Secret → **Konfigurēt ar Google** → **Aktīva**. `module_gmail_plugin` ieslēdzams tikai pēc tam. Fallback env: `GOOGLE_PLUGIN_CLIENT_ID`, `GOOGLE_PLUGIN_CLIENT_SECRET`. Pēc Client ID maiņas lietotājiem **Atjaunot Gmail**. Chrome unpacked spraudnim koda atjauninājums nav vajadzīgs.
...
Kad `module_google_drive` un `module_file_upload` ir ieslēgti, komandas `...` rāda **Google Drive Integrācija** lietotājiem ar `team.integrations.google_drive`. Lapa `/team/google-drive`: pieslēgt Google kontu (`drive.file`), mapes ceļš (jaunai integrācijai noklusējums = komandas nosaukums; esošās `Routine` mapes paliek). Pieslēgts konts = faili tiek augšupielādēti uz Drive (`store_on_server = false`: Routine glabā metadatus + `google_drive_file_id`, bez `content`; jaunie inserti vienmēr bez `content`, `123`; migrācija `132`). Konfigurēt var vadītājs / `team.integrations.google_drive` / `is_admin` (neatkarīgi no `team.settings.edit`). Refresh token: `team_google_drive_integrations` (tikai service role). Credentials no admin **Google OAuth** (`site_integrations`); fallback env: `GOOGLE_DRIVE_CLIENT_ID`, `GOOGLE_DRIVE_CLIENT_SECRET`. Redirect URI: `/auth/google-drive/callback` (tam pašam Google OAuth klientam). Google Cloud: ieslēgt Drive API. Augšupielāde: `POST /api/google-drive/upload` (XHR progress). Pirms mapes izveides Drive meklē esošu mapi ar to pašu nosaukumu tajā pašā vecākmapē (lai pēc atkārtotas pieslēgšanas nerastos `test (1)`); kešots ID, kas vairs nav mape, tiek izmests. Drive-only priekšskatījums/lejupielāde: `GET /api/google-drive/content?kind=list|task&id=…` (`download=1` → `Content-Disposition: attachment` ar ASCII `filename` + `filename*`); pārsaukšana: `POST /api/google-drive/rename`. Klienta URL palīgi `google-drive/content-url.ts` (statisks imports no file viewer / failu lapām, bez atsevišķa async chunk). UI overlay: `FileUploadOverlay`. Faila pārsaukšanā paplašinājums ir fiksēts (`renameKeepingExtension` / `NameFormModal` `nameSuffix`).

## OneDrive (komandas faili)

Kad `module_onedrive` un `module_file_upload` ir ieslēgti, komandas `...` rāda **OneDrive Integrācija** lietotājiem ar `team.integrations.onedrive`. Lapa `/team/onedrive`: pieslēgt Microsoft kontu (`Files.ReadWrite`), mapes ceļš (jaunai integrācijai noklusējums = komandas nosaukums). Pieslēgts konts = jaunie faili uz OneDrive (`132`). Konfigurēt: vadītājs / `team.integrations.onedrive` / `is_admin`. `module_onedrive` admin slēdzis ieslēdzams tikai ja **Microsoft OAuth** ir konfigurēts un ieslēgts (`/admin/integrations`). Credentials glabājas `site_integrations` (`microsoft_oauth`); fallback env: `ONEDRIVE_CLIENT_ID`, `ONEDRIVE_CLIENT_SECRET`. Redirect URI login/konfigurācijai: `/auth/microsoft-oauth/callback`; komandas pieslēgšanai: `/auth/onedrive/callback`. Refresh token: `team_onedrive_integrations`. OneDrive ir līdzvērtīgs Drive: metadati + `onedrive_file_id` (`124`), baiti paliek Microsoft (`123` strip `content`). Augšupielāde: `POST /api/onedrive/upload`; priekšskatījums/lejupielāde: `GET /api/onedrive/content`; pārsaukšana: `POST /api/onedrive/rename`. Kopīgais klients: `cloud-storage/queue-upload.ts` + `useTeamCloudStorage()` (kļūdu teksti no `message-key.ts` pēc ieslēgtajiem moduļiem).

## Chrome extension (Gmail)

Mapē `extensions/gmail` — unpacked Chrome MV3 (`manifest` `0.4.66`, permission `scripting`). Host piekļuvi prasa pirmajā popup/Gmail klikšķī (`plugin-permissions.js`) ar vienu `chrome.permissions.request` (bez `contains` pirms tam — tas pazaudē user-gesture). Unpacked (bez `update_url`) tajā pašā pieprasījumā iekļauj `localhost:3120` un `127.0.0.1:3120`, lai Chrome Details → Site access nav jāslēdz ar roku. Chrome Web Store būvējumā localhost **nav** šajā pieprasījumā: ja kāds origins nav piešķirams, Chrome nedod nevienu. Service worker **neizsauc** `chrome.permissions.request`. Nav `optional_host_permissions` (Chrome atmet dublikātus pret `host_permissions`). Popup: balta kartīte, avatars, vārds/uzvārds, e-pasts, **Iziet** tikai ikona augšējā labajā stūrī, centrēts loading, komandu pārslēgšana, Drive brīdinājums zem select, Gmail statuss kā ikona ar tooltip; bez URL/Client ID ievades; `12px` noapaļojums, `{SYSTEM_NAME}` no `getExtensionStrings(language, systemName)`. **Ikona:** pakotnes `icons/icon{16,48,128}.png` no `icons/logo.png` (ja nav logo - iniciāļi kā vietnes favicon); toolbar `chrome.action.setIcon` no admin augšupielādētā logotipa, citādi iniciāļi (`logo_color`). Pārģenerēt: `node extensions/gmail/scripts/generate-icons.mjs`. Zinātie origini (`KNOWN_SITE_ORIGINS` / `APP_ORIGIN_CANDIDATES`): `https://www.tasqin.com` (pirmais), `https://tasqin.com`, `http://localhost:3120`. **`getAppBase()`:** ja ir derīga sesija kādā originā — to izmanto; bez sesijas (Login) preferē production, ne `localhost`, pat ja lokālais serveris atbild uz `/api/extension/config`. Sesija ir spraudņa paša: `chrome.storage.local` (`extensionAuth` + `refresh_token`), API ar Bearer un `credentials: omit`. Vietnes cookie tikai bootstrap, ja storage tukšs un lietotājs nav izgājis no spraudņa; **Iziet** spraudnī nenoņem vietnes cookies. Vietnes **Iziet** (`signOutWebsiteLocally`) notīra tikai pārlūka cookie un aptur auto-refresh; **neizsauc** GoTrue `/logout`, jo `signOut({ scope: "local" })` tomēr atsauc šo sesiju serverī un nogalinātu spraudni, ja tokeni ir kopīgi. Google bootstrap (`bootstrap-from-ticket`) izveido **jaunu** spraudņa sesiju (`mintIndependentPluginSession`), lai vietnes sesija un spraudnis nebūtu vienā refresh ģimenē. Cookie bootstrap tikai ja `chrome.storage` ir tukšs. Cookie bootstrap lasa arī Supabase `base64-` chunked `sb-*-auth-token`. Alarm ~20 min (un Chrome `onStartup`) atjauno access tokenu; cookie un `chrome.storage` salīdzina pēc `expires_at` (jaunākā uzvar, vienādi paliek storage), lai vietnes tokenu rotācija nepārrakstītu spraudni ar vecāku cookie; `getAppBase` origin probe bez refresh; `chrome.storage` dzēš tikai spraudņa **Iziet**, ne 401 refresh (pēc invalid ir ~10 min cooldown, jaunāks cookie to noņem) (~30 dienas). Custom login (`POST /api/extension/login` ar `remember: true`) vai Google (`GET /auth/gmail-plugin/login` → Google konta izvēle → `/auth/gmail-plugin/done?logged_in=1`; done lapa rāda «Ienāci» tikai ar reālu cookie sesiju un ieliek `data-routine-bootstrap-ticket`; background apmaina ticket **vienreiz** (`POST /api/extension/bootstrap-from-ticket`, kešots); `plugin-auth.js` tikai `GET /api/extension/bootstrap-session` (ne ticket, lai nebūtu dubulta consume). Sesijas origin paliek tas pats — localhost JWT netiek sūtīts uz www). **Atjaunot Gmail:** `POST /api/extension/gmail-bridge-ticket` → `GET /auth/gmail-plugin/bridge?t=…` iestata pārlūka sesiju → `/auth/gmail-plugin/start` (Gmail OAuth). Ja bridge neizdodas, fallback `/start` + cookie sync; ja nonāk pie `/login?next=/auth/gmail-plugin/start`, login OAuth nodod `next` atpakaļ pie Gmail OAuth. Publisks `GET /api/extension/config` (logo, valoda, vai e-pasts/Google ieslēgts, `loginPath`; `?lang=` / `Accept-Language` viesiem). Ja Gmail nav savienots, **Savienot Gmail** (`/auth/gmail-plugin/start`) iet caur to pašu `/auth/google-oauth/callback` kā login (vecais `/auth/gmail-plugin/callback` paliek aliases); tokeni `user_gmail_connections` (service role, RLS deny). Komandai bez ieslēgtā mākoņa (Google Drive un/vai OneDrive pēc moduļiem) — popup rāda sarkanu brīdinājumu, Gmailā poga paliek un modālis paskaidro iemeslu (poga netiek slēpta). Ja mākonis ir pieslēgts, Gmailā inline logo poga → modālis → saraksts / mape / uzdevums → apakšuzdevums (vai jauns apakšuzdevums ar nosaukumu, sākumu, termiņu, statusu, atbildīgo un aprakstu). E-pasts un pielikumi no **atvērtās Gmail vēstules** (DOM + `download_url`; OAuth scope `gmail.addons.current.message.readonly`, bez `gmail.readonly`); e-pasta `.txt` nosaukums ir sūtītāja adrese (`sandris@gmail.com.txt`); limīts 25 MB. Sesijas access tokenu spraudnis atjauno ar `POST /api/extension/refresh` (tāpēc vairs “neizmet” pēc ~1 h). `proxy` `/api/extension/` apstrādā CORS (OPTIONS 204); kanoniskā hosta 301 šos ceļus izlaiž. Ielogotam UI valoda no `users.language_code` (`resolveExtensionUiLanguage`); pirms ielogošanās Chrome UI valoda (`chrome.i18n.getUILanguage`, `?lang=`). Sk. `extensions/gmail/README.md`.

**Ātrdarbība.** MV3 service worker mirst pēc ~30 s dīkstāves, tāpēc keši nedrīkst palikt tikai atmiņā. Verificētais app origin glabājas `chrome.storage.local` (`extensionAppBaseVerified`, 24 h) — kamēr tas ir svaigs, `getAppBase()` **nezondē** visus `APP_ORIGIN_CANDIDATES` ar `/api/extension/config`; tīkla kļūda `apiFetch` atzīmi notīra. Sesijas atbilde glabājas `chrome.storage.local` (`extensionSessionCache`, ne `session` — tas nodzēstos pārlūka restartā): svaiga 45 s, pēc tam līdz 10 min tiek atdota uzreiz ar fona revalidāciju (tikai ielogotam lietotājam), un `broadcastSessionUpdate` aizsūta rezultātu. **Popup** (`options.js`) šo ierakstu lasa **tieši no storage** un uzzīmē kontu, pirms service worker vispār pamostas (`readCachedSessionSnapshot`, ≤7 dienas vecs); tikai pēc tam iet `refreshUi({ silent: true })`, kas revalidē bez loading ekrāna un pārzīmē uz `routine.sessionUpdated`. `force: true` paliek darbību pogām (login, Iziet, komandas maiņa, Savienot Gmail). Bezsaistē silent revalidācija **nenoņem** jau uzzīmēto kontu. `content.js` `openModal()` izmanto šo kešu (hover prefetch `mouseenter` / `focus`) un pieprasa `force: true` **tikai** tad, ja kešotā atbilde vestu uz kļūdas vai reconnect ekrānu (`sessionReadyToAttach`). `format=full` Gmail ziņa tiek kešota vienā slotā (`extensionGmailMessage`, 5 min, ≤4 MB), lai **Pielikumi** saraksts un **Pievienot** nelejupielādētu to pašu ziņu divreiz. Serverī `/api/extension/session` iet vienā paralēlā vilnī, moduļu karodziņi ir viens `site_frontend_modules` `.in()` vaicājums, un `/api/extension/browse?step=subtasks` lasa statusu katalogu un parent rindu tikai vienreiz (`loadExtensionSubtasksBundle`).
...
**Reload / content script.** Pēc `chrome://extensions` → **Reload** vecā Gmail cilne zaudē kontekstu (`Extension context invalidated` pie `content.js` `sendMessage`). `content.js` `send()` to ķer un rāda `errors.extension_context_invalidated` (pārlādē Gmail ar F5). Pēc spraudņa Reload vienmēr **F5** Gmailā. Service worker Gmail cilnes **nepārinjicē** — pārinjicēts `content.js` noņemtu root lietotājam atvērtā modāļa vidū. Root un inline poga ir piesaistīti `chrome.runtime.id` (`routine-gmail-root-{id}`, `data-tasqin-gmail-inline`), lai Chrome Web Store kopija (vecie `#routine-gmail-root` / `data-routine-gmail-inline`) nenogalinātu unpacked UI.

**Izvēles saraksts.** Katrs solis (saraksti → mape/uzdevums → apakšuzdevums) saņem **jaunu** `<ul>` caur `freshResults()`, nevis `results.innerHTML = ""`. `#routine-gmail-results` ir `overflow-anchor: none`, un `resetResultsScroll()` paliek augšā arī pēc pielikumu ielādes. Inline poga uzreiz rāda sistēmas iniciāli (`applyInlineName`); logotips pārklāj burtu tikai pēc `img.onload`. Kešots nosaukums `Routine` tiek ignorēts.
...
| Tabula | Saturs |
|---|---|
| `teams` | Komandas (`team-…`); `payment_plan_id` / `until` / `paid` / `is_trial` / `is_early_bird` (`062`); `stripe_customer_id` / `stripe_subscription_id` / `paid_seat_count` / `billing_period` (`097`); `billing_cycle_end` (`098`); `early_bird_seat_count` (`099`); `is_vip` (`122`) |
...
| `site_settings` | Sistēmas nosaukums, slogans, `legal_email` (privātuma pārziņa kontakti un lietotāju kļūdu/atsauksmju/funkciju e-pasti, `084`), `logo_url` / `favicon_url` / `notification_favicon_url` (data URL; `138`), `logo_color`, datumu/laika noklusējums (`week_start_day`, `date_format`, `date_separator`, `time_format`), `timezone` (`Europe/Riga`, cron fallback, `091`); `payment_plans_enabled`, `trial_plan_id`, `trial_days`, `early_bird_limit` (`062`), `early_bird_claimed` (`099`); `docs_enabled` (`118`) |
| `site_docs_categories` | Publiskās dokumentācijas kategorijas (`slug`, ikona, `sort_order`, `is_visible`; `118`, `120`) |
...
| `site_docs_article_translations` | Rakstu nosaukums, slogans, Markdown saturs pēc valodas (`118`, `119`, `127` kopija no noklusējuma, `128`/`129` īstie tulkojumi) |
...
| `site_user_feedback` | Lietotāju kļūdu ziņojumi, funkciju pieprasījumi un atsauksmes (`kind`, `title`, `body`, `rating` 0–5 atsauksmēm `144`, `vote_count`; `088`); SELECT: `feature` visiem authenticated, pārējie tikai autoram |
...
| `work_lists` | Saraksti (`kind`, `is_private`, `default_access_level`, `created_by`, `custom_columns` `151`, `column_order` `152`) |
| `work_list_viewers` | Privāta saraksta biedri + `access_level` |
| `work_list_viewer_roles` | Saraksta lomu pieeja + `access_level` |
| `work_tasks` | Mapes, uzdevumi, apakšuzdevumi (`kind` + `parent_id` + `deleted_at` + `archived_at`; `status` = kataloga ID; `checklists` JSONB; `custom_fields` `151`; `hidden_status_ids`, `status_order`, `status_group_overrides` — uzdevuma līmeņa apakšuzdevumu statusu layout, `055`) |
...
| `task_files` | Apakšuzdevumu pielikumi; `google_drive_file_id` (`070`) un/vai `onedrive_file_id` (`124`); `note` tooltipam uz kartītes (`126`); bināriem bez `content` (`123`); maziem teksta failiem DB saturs atļauts (`137`); `has_content` (`073`) |
| `list_files` | Saraksta faili kokā; `google_drive_file_id` (`070`) un/vai `onedrive_file_id` (`124`); jaunie inserti bez `content` (`123`); `has_content` (`073`) |
...
Cookie `routine-app-cookie-consent` saglabā sīkdatņu piekrišanu (180 dienas). Cookie `routine-app-remember-session` (`1`/`0`) ir obligātā: vai Atcerēties mani ir ieslēgts. Supabase auth sīkdatnes paliek 30 dienas, ja `1`; bez tā - sesijas sīkdatnes. Cookie `routine-app-list-window-order` saglabā 3 logu kārtību katrā sarakstā, ja atļautas preferenču sīkdatnes. Cookie `routine-app-history-pane` saglabā mapes un apakšuzdevuma vēstures bloka redzamību. Cookie `routine-app-status-group-collapsed` saglabā statusu grupu sakļaušanu. Tabulas kolonnu redzamība un statusu grupu kārtošana ir `users.ui_preferences` (ne sīkdatne). Cookie `routine-app-language` + `routine-app-language-chosen` saglabā viesu apzināto UI valodu; bez `chosen` rāda sistēmas noklusējumu, pēc ielogošanās izvēle nonāk `users.language_code`.
