...
| Ceļš | Saturs |
|---|---|
| `/` | Landing (`LandingPage` + `LandingAppPreview`); H1/title 15 valodās, sekundārais CTA `#features`; hero eagers, zem fold `next/dynamic` + `LazyOnVisible`; hash saites `smooth-scroll.ts`; fīčas un hero no `resolveLandingPageContent`, FAQ no `resolveLandingFaqItems` (ieslēgtie frontend moduļi, t.sk. `module_send_file`); ja `payment_plans_enabled` un ir vismaz viens plāns — galvenes **Cenas** (`#pricing`), vienāda augstuma plānu kartītes (CTA apakšā, Early access/izmēģinājums birkās pie nosaukuma) + salīdzinājuma tabula (`landing-pricing.tsx`); ielogotam `redirect("/dashboard")` |
...
| Abonementi | `/team/billing` | Vadītāja vieta bez maksas; maksā tikai par vietām virs 1. Apmaksātās / aizņemtās / brīvās vietas, nākamais rēķins, checkout kopsavilkums (maksas lietotāji, summa, + PVN, Early access sadalījums), mēneša/gada toggle, **Iegādāties 1 vietu** (Stripe prorata līdz cikla beigām) pirms uzaicinājuma, ja nav brīvas vietas; gaidošie **Samaksāt** vecām `pending_payment` rindām; neizmantotā vieta cikla beigās nokrīt no nākamā rēķina; Checkout success ar `session_id` apstiprinājumu (mount guards pret dubultu toast); **Atteikties no abonementa** perioda beigās (`cancel_at_period_end`) un atjaunošana (zaļa **Atjaunot abonementu** poga arī `TeamSubscriptionEndingBanner`); salokāma abonementa pārvaldība kopsavilkuma kartē (`fa-angle-down`); `TeamSubscriptionEndingBanner` + `useTeamBillingLiveSync` (Stripe webhook/polling bez refresh); vadītājs / `team.settings.edit` / `is_admin` |
...
| `/admin/payment-plans` | `site_payment_plans` katalogs: ieslēgums **tikai ar Stripe**, izmēģinājums, Early access limīts, `max_members`, mēneša/gada cenas, frontend moduļi plānā (`AdminPaymentPlansForm`) |
...
app/
  ...
  components/
    ...
    admin-payment-plans-form.tsx  # Maksas plānu katalogs, max_members, cenas, izmēģinājums, Early access
  ...
  lib/
    ...
    payment-plans/                # helpers + repository + team-plan (katalogs, cenas, trial, Early access, moduļu filtrs)
