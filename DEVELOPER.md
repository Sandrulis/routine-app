...
- Datumi UI: `useDisplayPreferences().formatDate()` / `formatDateTime()`. Efektīvās preferences: lietotāja (`users.week_start_day`, `date_format`, `date_separator`, `time_format`, `timezone`) ja norādītas, citādi `site_settings`. Laika josla: lietotājs → serveris (`Europe/Riga`); datumi ar laiku caur `Intl` ar efektīvo joslu; tīri `YYYY-MM-DD` bez nobīdes. Noklusējums: pirmdiena, `d.m.Y`, `.`, 24 h.
...
| `task_files` | Apakšuzdevumu pielikumi; `google_drive_file_id` (`070`) un/vai `onedrive_file_id` (`124`); jaunie inserti bez `content` (`123`); `has_content` (`073`) |
