...
| `module_gmail_plugin` | Gmail Chrome spraudnis (`extensions/gmail` `0.4.7`): sesija, komandu pārslēgšana, e-pasta pievienošana. **Turpināt ar Google** → `/auth/gmail-plugin/login` (ne `/login`). Admin slēdzis tikai ja Google OAuth ir ieslēgts. Gmail OAuth tokeni `user_gmail_connections` (service role). Redirect `/auth/google-oauth/callback` | Spraudnis rāda, ka modulis izslēgts |
