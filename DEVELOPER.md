...
- `routine-app-list-window-order`, `routine-app-history-pane`, `routine-app-status-group-sort` un `routine-app-table-columns` raksta tikai ar `preferences` piekrišanu
...
Cookie `routine-app-cookie-consent` saglabā sīkdatņu piekrišanu (180 dienas). Cookie `routine-app-remember-session` (`1`/`0`) ir obligātā: vai Atcerēties mani ir ieslēgts. Supabase auth sīkdatnes paliek 30 dienas, ja `1`; bez tā - sesijas sīkdatnes. Cookie `routine-app-list-window-order` saglabā 3 logu kārtību katrā sarakstā, ja atļautas preferenču sīkdatnes. Cookie `routine-app-history-pane` saglabā mapes un apakšuzdevuma vēstures bloka redzamību. Cookie `routine-app-status-group-sort` saglabā statusu grupu kārtošanu (augoši / dilstoši). Cookie `routine-app-table-columns` saglabā tabulas kolonnu redzamību. Cookie `routine-app-language` + `routine-app-language-chosen` saglabā viesu apzināto UI valodu; bez `chosen` rāda sistēmas noklusējumu, pēc ielogošanās izvēle nonāk `users.language_code`.
