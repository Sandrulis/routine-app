...
| `/docs` | Publiskā dokumentācija (`DocsShell` layout + sānjosla, mobilā izvēlne); ieslēdz `/admin/docs`; kājene **Documentation** ja `docs_enabled` |
...
Indeksējamās lapas: `/`, `/privacy`, `/terms`, `/cookies`, `/login`, `/signup`, un `/docs` ja `docs_enabled`. Pārējais (dashboard, saraksti, komanda, admin, iestatījumi, uzaicinājumi, paroles atjaunošana, `/api/`, `/auth/`, kalendārs) ir `noindex` + `robots.txt` `Disallow` + `X-Robots-Tag: noindex, nofollow`.
...
| JSON-LD | landing `LandingJsonLd` (Organization, WebSite, SoftwareApplication + additionalType WebApplication, FAQPage; ja maksas plāni ieslēgti — Offer katram plānam uz `#pricing`) |
...
| `/admin/docs` | Publiskā dokumentācija (`site_docs_*`): kategorijas ar ikonu, apakškategorijas ar Markdown (kods, YouTube, attēli), slogans, redzamība, DnD `sort_order`; slēdzis `docs_enabled` rāda kājenes saiti Documentation. Saturs pagaidām noklusējuma valodā |
...
app/
  ...
  sitemap.ts                      # /sitemap.xml — landing + legal × visām LANGUAGE_CODES; `/docs` ja docs_enabled
  ...
  components/
    ...
    docs-shell.tsx                # Publiskais docs layout: sānjosla, mobilā burger izvēlne, raksta loading
    docs-sidebar.tsx              # Docs kategorijas/apakškategorijas; mobilajā overlay
    ...
    admin-docs-articles.tsx       # /admin/docs/[id]: apakškategorijas, saturs, attēli, DnD
  ...
  lib/
    ...
    docs/                         # Publiskā dokumentācija: tipi, repository, slug, attēli
    ...
    auth/                         # actions (generateLink + Resend HTML), email-password, generate-strong-password, password-strength, auth-confirm-link, MFA, OAuth, remember-session
...
supabase/migrations/              # 001–121: shēma, admin, work data, Drive, drošība, ielādes ātrums, e-pasta šabloni, Gmail spraudnis, publiskie login karogi, extra valodas, legal_email, invite preview `account_exists`, `file_forwarded` vēsture, Resend email id indekss, user feedback + feature votes, cron jobs, hashed tokeni, timezone batchi, Turnstile, payment plan max_members, free/paid plānu seed, Stripe vietu norēķini, brīvas vietas paziņojums, Early Bird vietu pool, `billing_due`, noklusējuma lomu labels visās valodās (`101`), publiskā dokumentācija (`118`–`121`)
