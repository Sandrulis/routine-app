...
- Komandas koks paliek redzams; klikšķis uz **Komanda** un lietotāju rindām atver `/team` (`canOpenTeamPage` vienmēr atļauj). `+` rādās tikai ar `team.invite` (`useStartTeamInvite`); `...` tikai ar `team.options` un vismaz vienu iekšējo opciju (lomas, veidnes vai Drive).
...
| `/admin/docs` | Publiskā dokumentācija (`site_docs_*`): kategorijas ar ikonu, apakškategorijas ar Markdown (kods, YouTube, attēli), slogans, redzamība, DnD `sort_order`; slēdzis `docs_enabled` rāda kājenes saiti Documentation. Saturs pagaidām noklusējuma valodā. Poga ievieto `{SYSTEM_NAME}`; publiski tas kļūst par `site_settings.system_name` treknrakstā un lielajiem burtiem (`app/lib/docs/placeholders.ts`) |
...
app/
  ...
  components/
    ...
    admin-docs-articles.tsx       # /admin/docs/[id]: apakškategorijas, saturs, attēli, DnD, `{SYSTEM_NAME}` ievietošana; loading, kad atver rakstu; FileUploadOverlay augšupielādei
  ...
  lib/
    ...
    docs/                         # Publiskā dokumentācija: tipi, repository, slug, attēli, `{SYSTEM_NAME}` placeholders
