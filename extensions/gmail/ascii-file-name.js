/* Shared by Gmail content.js and background.js. Keep in sync with app/lib/format/ascii-file-name.ts */
(function (root) {
const LETTER_MAP = {
  ß: "ss",
  ẞ: "Ss",
  ...
  function cleanAsciiSegment(value) {
  ...
  function asciiSafeFileName(name, fallback) {
    const safeFallback = fallback || "file";
    ...
    const asciiBase = cleanAsciiSegment(base) || safeFallback;
    const asciiExt = cleanAsciiSegment(ext).replace(/_/g, "");
    const joined = asciiExt ? `${asciiBase}.${asciiExt}` : asciiBase;
    return joined.slice(0, 180) || safeFallback;
  }

  root.decodePercentEncodedName = decodePercentEncodedName;
  root.transliterateToAscii = transliterateToAscii;
  root.asciiSafeFileName = asciiSafeFileName;
